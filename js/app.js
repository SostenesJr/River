...
ROTAS.forEach(r => r.municipios.forEach((m, i) => {
  const rec = {
   ...
   RIVER OPS — TRIAGEM — APP.JS (VERSÃO ATUALIZADA COMPLETA)
   ============================================================ */
...
// Vincula o fechamento manual sem conflitos
...
/* AJUSTE RESPONSIVO: Aloca os dados simultaneamente na lista visível e na gêmea oculta do print */
function renderTabelaPortos() {
  ['esc', 'rod', 'v-rod'].forEach(p => {
    ...
    manifestoPortos[p].forEach(hit => {
      ...
      // Monta lista visível confortável para mexer no celular
      ...
      // Monta lista limpa horizontal widescreen para o print do WhatsApp
...
/* AJUSTE MESTRE: Mapeamento dinâmico automático de 100% das calhas do arquivo de dados */
function resetarTabelaPortos() {
  ...
  ROTAS.forEach(r => {
    r.municipios.forEach(m => {
      ...
      // Calhas Terrestres e Rodoviárias (H e I) -> RODOVIÁRIO
      if (r.num === 'H' || r.num === 'I') {
        manifestoPortos['v-rod'].push(hit);
      } 
      // Calhas de Médio e Alto Solimões (D e E) -> PORTO ROADWAY
      else if (r.num === 'D' || r.num === 'E') {
        manifestoPortos.rod.push(hit);
      } 
      // Restante das calhas logísticas fluviais (A, B, C, F, G, J) -> PORTO ESCADARIA
      else {
...
/* ── INICIALIZAÇÃO CORRIGIDA (CARGA MESTRE COMPLETA) ──────────
   AJUSTE DE FLUXO: Sincroniza a montagem da árvore mestre antes da distribuição de nós
   ------------------------------------------------------------ */
bRO();
resetarTabelaPortos();
renderRecentes();
