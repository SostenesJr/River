...
function lookup(cep5){
  ...
  const boats=emb?[...emb.e].sort((a,b)=>(nD(a.d)??99)-(nD(b.d)??99)):[];
  const aP=prev
...
function buildGuideHTML(){
  ...
  let body='<h1>Guia RIVER - Triagem Sequencial por Calha</h1>'+'<div class="sub">Sigla SLAM / CEP -> Rota -> Posicao | '+(new Date().toLocaleDateString('pt-BR'))+'</div><div class="grid">';
  ROTAS.forEach(r=>{
  ...
  body+='</div>';
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Guia RIVER</title><style>'+css+'</style></head><body>'+body+'</body></html>';
}
function printGuide(){
  ...
  if(!ov){
    ...
    bar.innerHTML='<span style="font-size:13px;font-weight:700">Guia RIVER - use o menu do navegador para imprimir</span>'+
      '<button onclick="document.getElementById(\'guide-overlay\').remove()" style="background:rgba(0,0,0,.3);border:none;color:#fff;padding:6px 12px;border-radius:5px;cursor:pointer;font-weight:600">&#10005; Fechar</button>';
...
function renderMap(){
  ...
  defs.innerHTML='<pattern id="gr" width="30" height="30" patternUnits="userSpaceOnUse"><path d="M30 0L0 0 0 30" fill="none" stroke="#0d1520" stroke-width=".4"/></pattern><filter id="gw"><feGaussianBlur stdDeviation="1.8" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter><style>.cm-dot{transition:transform .15s ease;transform-box:fill-box;transform-origin:center}.cm-marker:hover .cm-dot{transform:scale(2.1)}.cm-label{transition:opacity .12s ease;opacity:0;pointer-events:none}.cm-marker:hover .cm-label{opacity:1}</style>';
  svg.appendChild(defs);
  
  const g=document.createElementNS(NS,'g');g.id='mg';g.setAttribute('transform','translate('+T.x+','+T.y+') scale('+T.s+')');
  const bg=document.createElementNS(NS,'rect');bg.setAttribute('width',W);bg.setAttribute('height',H);bg.setAttribute('fill','#0e1a2b');g.appendChild(bg);
  ...
  const border=document.createElementNS(NS,'polygon');border.setAttribute('points',bPts);border.setAttribute('fill','#142840');border.setAttribute('stroke','#2a4a6e');border.setAttribute('stroke-width','1.5');g.appendChild(border);
  
  RIOS.forEach(rv=>{
    const pts=rv.coords.map(([lat,lng])=>{const p=proj(lat,lng);return p.x+' '+p.y;});
    const path=document.createElementNS(NS,'polyline');path.setAttribute('points',pts.join(', '));path.setAttribute('fill','none');path.setAttribute('stroke','#1e4a7a');path.setAttribute('stroke-width',rv.w);path.setAttribute('opacity',rv.op);g.appendChild(path);
  });
  ...
  const hl=document.createElementNS(NS,'text');hl.setAttribute('x',hub.x+10);hl.setAttribute('y',hub.y+4);hl.setAttribute('font-size','11');hl.setAttribute('fill','#14b8a6');hl.setAttribute('font-weight','700');hl.textContent='MANAUS';hg.appendChild(hl);g.appendChild(hg);

  ROTAS.forEach(r=>{
    const isRoad=(r.num===9||r.num===10);const pts=r.municipios.map(m=>LATLNG[m.cep]?proj(LATLNG[m.cep].lat,LATLNG[m.cep].lng):null).filter(Boolean);
    if(pts.length){
      const lineCoords=[[hub.x,hub.y],...pts.map(p=>[p.x,p.y])];const polyPts=lineCoords.map(p=>p[0]+' '+p[1]).join(', ');
      const line=document.createElementNS(NS,'polyline');line.setAttribute('points',polyPts);line.setAttribute('fill','none');line.setAttribute('stroke',r.cor);line.setAttribute('stroke-width',focR&&focR!==r.num?'1':'2');line.setAttribute('opacity',focR&&focR!==r.num?'0.05':'0.5');if(isRoad)line.setAttribute('stroke-dasharray','6,4');g.appendChild(line);
    }
  });

  ROTAS.forEach(r=>{
    r.municipios.forEach(m=>{
      const ll=LATLNG[m.cep];if(!ll)return;const p=proj(ll.lat,ll.lng);const dim=focR&&focR!==r.num;
      const grp=document.createElementNS(NS,'g');grp.setAttribute('class','cm-marker');grp.style.cursor='pointer';grp.setAttribute('opacity',dim?'0.06':'1');
      ...
      const nt=document.createElementNS(NS,'text');nt.setAttribute('x',p.x);nt.setAttribute('y',p.y+2.6);nt.setAttribute('text-anchor','middle');nt.setAttribute('font-size','6');nt.setAttribute('font-weight','700');nt.setAttribute('fill','#fff');nt.textContent=String(m.seq).padStart(2,'0');dot.appendChild(nt);grp.appendChild(dot);
      const lbl=document.createElementNS(NS,'text');lbl.setAttribute('class','cm-label');lbl.setAttribute('x',p.x+9);lbl.setAttribute('y',p.y+3.5);lbl.setAttribute('font-size','8');lbl.setAttribute('fill',dim?'#0e1a2b':'#bcd4dd');lbl.textContent=m.nome.split(' ')[0];grp.appendChild(lbl);
      
      if(window.innerWidth<768){grp.addEventListener('touchend',e=>{e.preventDefault();e.stopPropagation();oMS(m,r);});}
      else{
...
function sMT(e,c,rArg){
  ...
  const nb=rm?bB(rm.nome):null;const nx=nL(nb?nb.days:null);const tt=document.getElementById('mtt');
  tt.innerHTML='<div style="padding:10px 12px"><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><div style="min-width:38px;height:30px;border-radius:6px;background:'+color+';display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#fff;padding:0 6px"><div style="font-size:7px;opacity:.7">R'+String(c.rota).padStart(2,'0')+'</div><div>'+String(c.seq).padStart(2,'0')+'</div></div><div><div style="font-size:13px;font-weight:700">'+c.nome+'</div><div style="font-size:10px;color:'+color+'">'+(r?r.nome:'')+'</div></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:10px;margin-bottom:6px"><div>Anterior: '+(prev?prev.nome:'Inicio')+'</div><div>Proximo: '+(next?next.nome:'Fim')+'</div></div>'+(nb?'<div style="font-size:10px;display:flex;align-items:center;gap:6px"><span class="'+nx.c+'" style="font-size:9px;font-weight:700;padding:1px 6px;border-radius:3px">'+nx.l+'</span><span>'+nb.boat.n+'</span></div>':'')+'</div>';
  const mc=document.getElementById('sc-m').getBoundingClientRect();let lx=e.clientX-mc.left+14,ty=e.clientY-mc.top-12;if(lx+230>mc.width)lx-=244;if(ty+120>mc.height)ty-=120;tt.style.left=lx+'px';tt.style.top=ty+'px';tt.className='on';
}
function oMS(c,rArg){
  const r=rArg||gR(c.rota);const color=r?r.cor:'#14b8a6';const ri=r?r.municipios.findIndex(m=>m.seq===c.seq):-1;const rm=ri>=0?r.municipios[ri]:null;
  const prev=ri>0?r.municipios[ri-1]:null;const next=(ri>=0&&ri<r.municipios.length-1)?r.municipios[ri+1]:null;
...
function renderSelBar(){
  ...
  bar.classList.add('on');cnt.textContent=keys.length+' selecionados';
  chips.innerHTML=keys.map(k=>{const {mun,rota}=SEL[k];return '<div class="sel-chip"><span class="sel-chip-nm">'+mun.nome+'</span></div>';}).join('');
...
function openSelPanel(){
  ...
  Object.values(byRota).forEach(({rota,muns})=>{
    let html='<div class="sp-rota-head">'+rota.nome+'</div>';
    muns.forEach(m=>{html+='<div class="sp-mun">'+m.nome+' - Pos: '+m.seq+'</div>';});
    const block=document.createElement('div');block.className='sp-rota-block';block.innerHTML=html;body.appendChild(block);
  });
...
function mrowClick(e,cepK,rNum,mSeq){
  const rota=ROTAS.find(r=>r.num===rNum);const mun=rota.municipios.find(m=>m.seq===mSeq);
  const rect=e.currentTarget.getBoundingClientRect();const clickX=e.clientX-rect.left;
  if(Object.keys(SEL).length>0||clickX<28){toggleSel(cepK,mun,rota);const chk=document.getElementById('chk_'+cepK);if(chk)chk.innerHTML=SEL[cepK]?'✓':'';}
  else{lookup(mun.cep);SS('t',document.querySelector('.htab.on'));}
}

function SS(name,btn,mid){
  cur=name;['t','r','m','p'].forEach(s=>{const el=document.getElementById('sc-'+s);if(s===name)el.className='scr';else el.className='scr h';});
  if(btn){document.querySelectorAll('.htab').forEach(b=>b.classList.remove('on'));btn.classList.add('on');}
  cSh();if(name==='m'){renderMap();bL();updL();}
}
function cSh(){document.getElementById('sh').classList.remove('on');}
function setup(){const m=window.innerWidth<768;document.getElementById('btabs').style.display=m?'block':'none';document.getElementById('app').style.bottom=m?'58px':'0';document.querySelector('.htabs').style.display=m?'none':'flex';}
